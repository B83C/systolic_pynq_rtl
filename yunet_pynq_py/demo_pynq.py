...
parser.add_argument('--camera', '-c', type=int, default=0,
                    help='Usage: Specify camera device ID, defaults to 0.')
...
if __name__ == '__main__':
    ...
    if args.input is not None:
        ...
        # Pad to multiples of 32 (required for OpenCV 4.5.x compatibility)
        padded, orig_h, orig_w = pad_to_mult32(image)

        # Inference
        model.setInputSize([padded.shape[1], padded.shape[0]])
        results = model.infer(padded)
        ...
        while cv.waitKey(1) < 0:
            ...
            # Pad to multiples of 32 (required for OpenCV 4.5.x compatibility)
            padded, _, _ = pad_to_mult32(frame)
