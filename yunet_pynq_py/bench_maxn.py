"""Find max N — one-time A/C load, vary B size."""
...
w_pad = np.zeros((Cp, Kp), dtype=np.int8); w_pad[:Co, :Ci] = w_2d
A_bm = np.ascontiguousarray(w_pad.reshape(Cp//T, T, Kp//T, T).transpose(0, 2, 1, 3))
...
print(f"16×16 weight, A/C pre-loaded. Testing B stream throughput:")
print(f"{'N':>8s} | {'Time(ms)':>9s} {'Diff':>5s} {'MOPS':>7s}")
print("-"*35)

for N in [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 102400]:
    Np = ((N + T - 1) // T) * T
    act = rng.integers(-128, 127, (1, N, Ci), dtype=np.int8)
    b_pad = np.zeros((Np, Kp), dtype=np.int8); b_pad[:N, :Ci] = act.reshape(N, Ci)
    bb = allocate(shape=(Np*Kp,), dtype=np.int8); bb[:] = b_pad.reshape(-1)
    ob = allocate(shape=(Np*Cp,), dtype=np.int8)

    in_pa = bb.device_address; out_pa = ob.device_address
    nB = Np * Kp; nO = Np * Cp

    # Warmup + check correctness
    r = subprocess.run(["./sa_pointwise", "0", hex(in_pa), hex(out_pa), "0", str(nB), str(nO)],
    ...
    if d > 1:
        print(f"{N:8d} | FAIL at N={N} (diff={d})")
    ...
    for _ in range(3):
        r = subprocess.run(["./sa_pointwise", "0", hex(in_pa), hex(out_pa), "0", str(nB), str(nO)],
                           capture_output=True, text=True, timeout=30)
        ts.append(float(r.stdout.strip()))
    t = np.median(ts)
    ops = 2 * N * Ci * Co
    print(f"{N:8d} | {t*1000:8.4f} {d:5d} {ops/t/1e6:7.1f}")
    bb.freebuffer(); ob.freebuffer()
