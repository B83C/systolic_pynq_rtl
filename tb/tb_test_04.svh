...
task test_04_accumulation_accout1();
  ...
  if (out_count >= SIZE)
    for (int r = 0; r < SIZE; r++) check_row_str($sformatf("B1 row %0d", r), r, exp_B1[r]);

  if (out_count >= 2 * SIZE) begin
    $display("  Checking B1+B2 (accumulated) at %0d..%0d", SIZE, 2 * SIZE - 1);
    for (int r = 0; r < SIZE; r++) check_row_str($sformatf("acc row %0d", r), SIZE + r, exp_acc[r]);
  end
