...
module chlast_to_tiled_gather_sv #(
  ...
  localparam unsigned BUF_IDX_W = (CH_PER_BEAT * DATA_WIDTH > 1) ? $clog2(
      CH_PER_BEAT * DATA_WIDTH
  ) : 1;
  localparam GATHER_IDX_W = (OUT_COL * DATA_WIDTH > 1) ? $clog2(OUT_COL * DATA_WIDTH) : 1;
  ...
  always @(posedge clk, negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (axil_wr_en) begin
        case (s_axil_awaddr)
          REG_CT_CH:
          cfg_channels <= (s_axil_wdata[CFG_CH_W-1:0] == 0 || s_axil_wdata[CFG_CH_W-1:0] > MAX_CHANNELS)
                                          ? MAX_CHANNELS : s_axil_wdata[CFG_CH_W-1:0];
          REG_CT_RPT:
          repeat_cnt   <= (s_axil_wdata[REPLAY_CNT_W-1:0] == 0)
